...
void MysqlConnectionPool::Init(const std::string &host, unsigned short port,
    ...
    std::thread(&MysqlConnectionPool::RecycleMysqlConnectionTask, this)
        .detach();
}
...
void MysqlConnectionPool::ProduceConnectionTask() {
    while (isrunning_) {
        ...
        size_t new_connections =
            std::min(static_cast<size_t>(maxsize_) - total_connections,
