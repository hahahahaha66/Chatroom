                 ...
                 const std::string& password, const std::string& name)
{
    ...
    if (res) 
    {
        LOG_INFO << "Successfully connected to MySQL server at " 
                 << host << ":" << port;
        return true;
    } 
    else 
    {
        LOG_ERROR << "Failed to connect to MySQL server at "
                 << host << ":" << port << ": " 
                 << mysql_error(conn_);
        return false;
    }
...
{
    lastUsedTime_ = duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count();
}

long long MysqlConnection::GetLastTime()
{
    return duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count() - lastUsedTime_;
}