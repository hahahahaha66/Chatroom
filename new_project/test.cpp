...
class Warning {
  public:
    ...
    void printStackTrace() {
        vector<string> syms = {"panic+0x123/0x456",
    ...
    void simulatePanic() {
        ...
        // 关键 Panic 信息
        printf(
            "\n[    0.000000] Kernel panic - not syncing: Fatal exception in "
            "interrupt\n");
        printf("[    0.000001] CPU: %d PID: %d Comm: %s Tainted: %s\n",
               rand() % 8, 2 + rand() % 5000, "kworker/0:1",
               "G    6.1.33-arch1-1 #1");
        printf("[    0.000002] Hardware name: QEMU Standard PC (Q35 + ICH9, "
               "2009)\n");
        ...
        vector<string> extraLogs = {
            "EXT4-fs warning: mounting fs with errors, running journal "
            "recovery",
            ...
            "oom-killer: killing process 3456 (chrome) score 987 or sacrifice "
            "child",
            "PCIe Bus Error: severity=Corrected, type=Data Link Layer, "
            "(Transmitter ID)",
            "audit: type=1400 audit(0.123:45): apparmor=\"DENIED\" "
            "operation=\"open\" profile=\"snapd\" name=\"/etc/shadow\"",
            "NETDEV WATCHDOG: eth0: transmit queue 0 timed out",
            "systemd-journald[1]: Failed to rotate journal: Input/output "
            "error"};
        // 随机刷屏 80 行
        for (int i = 0; i < 80; ++i) {
