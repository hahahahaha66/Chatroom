...
FileUploader::FileUploader(EventLoop *loop, const std::string &serverip,
    ...
    : loop_(loop),
      ...
      filename_(filename), filepath_(filepath + '/' + filename),
      senderid_(senderid), receiverid_(receiverid), type_(type), sent_(0),
    ...
    client_.setConnectionCallback(
        std::bind(&FileUploader::OnConnection, this, _1));
    client_.setMessageCallback(
        std::bind(&FileUploader::OnMessage, this, _1, _2, _3));
    client_.setWriteCompleteCallback(
        std::bind(&FileUploader::OnWriteComplete, this, _1));
}

void FileUploader::Start() { client_.connect(); }
...
void FileUploader::OnMessage(const TcpConnectionPtr &conn, Buffer *buffer,
                             Timestamp time) {
...
FileDownloader::FileDownloader(EventLoop *loop, const std::string &serverip,
                               uint16_t serverport, const std::string &filename,
    ...
    : loop_(loop),
      ...
      savepath_(savedir + "/" + filename), filesize_(0), received_(0),
      gotheader_(false) {
    client_.setConnectionCallback(
        std::bind(&FileDownloader::OnConnection, this, _1));
    client_.setMessageCallback(
        std::bind(&FileDownloader::OnMessage, this, _1, _2, _3));
    client_.setWriteCompleteCallback([](const TcpConnectionPtr conn) {});
}

void FileDownloader::Start() { client_.connect(); }
...
void FileDownloader::SendDownloadRequest() {
    json js = {{"filename", filename_}, {"timestamp", timestamp_}};
...
void FileDownloader::OnMessage(const TcpConnectionPtr &conn, Buffer *buffer,
                               Timestamp time) {
    if (!gotheader_) {
        auto msgopt = codec_.tryDecode(buffer);
        if (!msgopt.has_value()) {
