...
void Client::start() { client_.connect(); }
...
Client::Client(EventLoop &loop, uint16_t port, std::string ip, std::string name)
    ...
    RegisterHandlerSafe(dispatcher_, "SendMessageBack", *this,
                        &Client::SendMessageBack);
    RegisterHandlerSafe(dispatcher_, "RecvMessage", *this,
                        &Client::RecvMessageBack);
    RegisterHandlerSafe(dispatcher_, "GetChatHistoryBack", *this,
                        &Client::GetChatHistoryBack);
    RegisterHandlerSafe(dispatcher_, "GetGroupHistoryBack", *this,
                        &Client::GetGroupHistoryBack);
    RegisterHandlerSafe(dispatcher_, "ChanceInterFaceBack", *this,
    ...
    RegisterHandlerSafe(dispatcher_, "ListFriendApplyBack", *this,
                        &Client::ListFriendApplyBack);
    RegisterHandlerSafe(dispatcher_, "ListSendFriendApplyBack", *this,
                        &Client::ListSendFriendApplyBack);
    RegisterHandlerSafe(dispatcher_, "ProceFriendApplyBack", *this,
                        &Client::ProceFriendApplyBack);
    RegisterHandlerSafe(dispatcher_, "ListFriendBack", *this,
                        &Client::ListFriendBack);
    RegisterHandlerSafe(dispatcher_, "BlockFriendBack", *this,
                        &Client::BlockFriendBack);
    RegisterHandlerSafe(dispatcher_, "DeleteFriendBack", *this,
                        &Client::DeleteFriendBack);
    RegisterHandlerSafe(dispatcher_, "CheckFriendBlockBack", *this,
                        &Client::CheckFriendBlockBack);

    RegisterHandlerSafe(dispatcher_, "CreateGroupBack", *this,
                        &Client::CreateGroupBack);
    RegisterHandlerSafe(dispatcher_, "SendGroupApplyBack", *this,
                        &Client::SendGroupApplyBack);
    RegisterHandlerSafe(dispatcher_, "ListGroupApplyBack", *this,
                        &Client::ListGroupApplyBack);
    RegisterHandlerSafe(dispatcher_, "ListSendGroupApplyBack", *this,
                        &Client::ListSendGroupApplyBack);
    RegisterHandlerSafe(dispatcher_, "ListGroupMemberBack", *this,
                        &Client::GroupMemberBack);
    RegisterHandlerSafe(dispatcher_, "ListGroupBack", *this,
                        &Client::ListGroupBack);
    RegisterHandlerSafe(dispatcher_, "QuitGrouprBack", *this,
                        &Client::QuitGroupBack);
    RegisterHandlerSafe(dispatcher_, "ChangeUserRoleBack", *this,
                        &Client::ChangeUserRoleBack);
    RegisterHandlerSafe(dispatcher_, "ProceGroupApplyBack", *this,
                        &Client::ProceGroupApplyBack);
    RegisterHandlerSafe(dispatcher_, "DeleteGroupBack", *this,
                        &Client::DeleteGroupBack);
    RegisterHandlerSafe(dispatcher_, "BlockGroupUserBack", *this,
                        &Client::BlockGroupUserBack);
    RegisterHandlerSafe(dispatcher_, "RemoveGroupUserBack", *this,
                        &Client::RemoveGroupUserBack);
    RegisterHandlerSafe(dispatcher_, "AddFriendToGroupBack", *this,
                        &Client::AddFriendToGroupBack);

    RegisterHandlerSafe(dispatcher_, "GetFileServerPortBack", *this,
                        &Client::GetFileServerPortBack);
    RegisterHandlerSafe(dispatcher_, "ListFriendFileBack", *this,
                        &Client::ListFriendFileBack);
    RegisterHandlerSafe(dispatcher_, "ListGroupFileBack", *this,
                        &Client::ListGroupFileBack);

    RegisterHandlerSafe(dispatcher_, "ChancePasswordBack", *this,
                        &Client::ChancePasswordBack);
    RegisterHandlerSafe(dispatcher_, "ChanceUserNameBack", *this,
                        &Client::ChanceUserNameBack);
}
...
void Client::MessageCompleteCallback(const TcpConnectionPtr &conn) {
    if (conn->connected()) {
        // LOG_INFO << conn->getLoop() << " Loop and send messages to " <<
        // conn->peerAddress().toIpPort();
    }
}
...
void Client::Login(const json &js) { this->send(codec_.encode(js, "Login")); }
...
void Client::FlushBack(const TcpConnectionPtr &conn, const json &js) {
    ...
    if (end) {
        ...
        for (auto it : js["friends"]) {
            ...
            newfriendlist[friendid] =
                Friend(friendid, friendname, friendemail, online, block);
        ...
        for (auto it : js["friendapply"]) {
            ...
            newfriendapplylist.emplace(
                friendid,
                FriendApply(friendid, friendname, applystatus, email));
        }
        friendapplylist_ = std::move(newfriendapplylist);
        ...
        for (auto its : js["groups"]) {
            ...
            for (auto it : its["groupuser"]) {
                ...
                groupuserlist.emplace(userid,
                                      GroupUser(userid, username, role, mute));
            }
    ...
    if (end) {
        ...
        for (auto it : js["messages"]) {
            ...
            if (senderid == userid_) {
                ...
                std::cout << timestamp << " " << friendname << ": " << content
                          << std::endl;
            }
        }
    } else {
...
void Client::GetGroupHistoryBack(const TcpConnectionPtr &conn, const json &js) {
    ...
    if (end) {
        ...
        for (auto it : js["messages"]) {
            ...
            if (senderid == userid_) {
                std::cout << timestamp << " " << name_ << ": " << content
                          << std::endl;
            } else {
                if (news && status == "Unread") {
                    std::cout << "以下为新消息" << std::endl;
                ...
                sendername =
                    grouplist_[groupid].groupuserlist_[senderid].username_;
                std::cout << timestamp << " " << sendername << ": " << content
                          << std::endl;
            }
        }
    } else {
...
void Client::NoticeMessage(const TcpConnectionPtr &conn, const json &js) {
    std::string type;
    int senderid;
    AssignIfPresent(js, "type", type);
    AssignIfPresent(js, "senderid", senderid);
    if (type == "Group") {
        std::string groupname = grouplist_[senderid].name_;
        std::cout << "你有来自群聊" << groupname << "的新消息" << std::endl;
    } else if (type == "Private") {
        std::string friendname = friendlist_[senderid].name_;
        std::cout << "你有来自好友" << friendname << "的新消息" << std::endl;
    } else if (type == "Apply") {
        std::cout << "你有新的好友申请" << std::endl;
    } else {
...
void Client::ListFriendApplyBack(const TcpConnectionPtr &conn, const json &js) {
    ...
    if (end) {
        ...
        for (auto it : js["friendapplys"]) {
            ...
            AssignIfPresent(it, "status", status);
            AssignIfPresent(it, "email", email);
            newfriendapplylist_.emplace(
                fromid, FriendApply(fromid, fromname, status, email));
        }
...
void Client::ListSendFriendApplyBack(const TcpConnectionPtr &conn,
                                     const json &js) {
    bool end = false;
    AssignIfPresent(js, "end", end);
    ...
    if (end) {
        ...
        for (auto it : js["friendsendapplys"]) {
            ...
            AssignIfPresent(it, "status", status);
            AssignIfPresent(it, "email", email);
            newfriendsendapplylist_.emplace(
                targetid, FriendApply(targetid, targetname, status, email));
        }
...
void Client::ProceFriendApplyBack(const TcpConnectionPtr &conn,
                                  const json &js) {
    bool end = false;
...
void Client::CheckFriendBlock(const json &js) {
    this->send(codec_.encode(js, "CheckFriendBlock"));
}

void Client::CheckFriendBlockBack(const TcpConnectionPtr &conn,
                                  const json &js) {
    bool end = false;
    AssignIfPresent(js, "end", end);

    if (end) {
        std::cout << "你已被屏蔽" << std::endl;
        tempbool = true;
    } else {
    }
...
void Client::ListGroupApplyBack(const TcpConnectionPtr &conn, const json &js) {
    ...
    if (end) {
        ...
        for (auto it : js["groupapplys"]) {
            ...
            AssignIfPresent(it, "status", status);
            std::cout << "申请人id" << "  申请人名字   " << "  状态"
                      << std::endl;
            std::cout << applyid << "     " << applyname << "     " << status
                      << std::endl;
        }
    } else {
...
void Client::ListSendGroupApplyBack(const TcpConnectionPtr &conn,
                                    const json &js) {
    bool end = false;
    AssignIfPresent(js, "end", end);
    ...
    if (end) {
        ...
        for (auto it : js["groupsendapplys"]) {
            ...
            AssignIfPresent(it, "status", status);
            std::cout << "申请群聊id" << "  申请群聊名字   " << "  状态"
                      << std::endl;
            std::cout << groupid << "     " << groupname << "     " << status
                      << std::endl;
        }
    } else {
...
void Client::AddFriendToGroup(const json &js) {
    this->send(codec_.encode(js, "AddFriendToGroup"));
}

void Client::AddFriendToGroupBack(const TcpConnectionPtr &conn,
                                  const json &js) {
    bool end = false;
    AssignIfPresent(js, "end", end);

    if (end) {
        std::cout << "拉好友进群成功" << std::endl;
    } else {
        std::cout << "拉好友进群失败" << std::endl;
    }
...
void Client::GetFileServerPortBack(const TcpConnectionPtr &conn,
                                   const json &js) {
    bool end = true;
...
void Client::UploadFile(std::string filename, std::string filepath,
                        int receiverid, std::string type) {
    waitingback_ = true;
    json js = {};
    ...
    uploader_ =
        std::make_shared<FileUploader>(loop_, ip_, fileserverport_, filename,
                                       filepath, userid_, receiverid, type);
    uploader_->Start();
}

void Client::DownloadFile(std::string filename, std::string savepath,
                          std::string timestamp) {
    waitingback_ = true;
    json js = {};
    GetFileServerPort(js);
    ...
    downloader_ = std::make_shared<FileDownloader>(
        loop_, ip_, fileserverport_, filename, savepath, timestamp);
    downloader_->Start();
}

void Client::ChancePassword(const json &js) {
    this->send(codec_.encode(js, "ChancePassword"));
}

void Client::ChanceUserName(const json &js) {
    this->send(codec_.encode(js, "ChanceUserName"));
}

void Client::ChancePasswordBack(const TcpConnectionPtr &conn, const json &js) {
    bool end = false;
    AssignIfPresent(js, "end", end);

    if (end) {
        std::string newpassword;
        AssignIfPresent(js, "newpassword", newpassword);
...
void Client::ChanceUserNameBack(const TcpConnectionPtr &conn, const json &js) {
    bool end = false;
    AssignIfPresent(js, "end", end);

    if (end) {
        std::string newname;
        AssignIfPresent(js, "newname", newname);
        name_ = newname;
        std::cout << "修改名字成功" << std::endl;
    } else {
        std::cout << "修改名字失败" << std::endl;
    }
...
void Client::InputLoop() {
    while (!exitflag_) {
        if (currentState_ == "InitMenu") {
        ...
        } else if (currentState_ == "MainMenu") {
            json js = {{"userid", userid_},
                       {"interfaceid", -1},
                       {"interface", "Notice"}};
            waitingback_ = true;
            ...
            int order = 0;
            std::string input;
            std::cout << "1. 消息中心" << std::endl;
            std::cout << "2. 好友管理" << std::endl;
            std::cout << "3. 群聊管理" << std::endl;
            ...
            if (order == 1) {
                ...
                currentState_ = "FriendManage";
                ClearScreen();
                continue;
            } else if (order == 3) {
                currentState_ = "GroupManage";
                ClearScreen();
                continue;
            } else if (order == 4) {
                currentState_ = "PersonalCenter";
                ClearScreen();
                continue;
            } else if (order == 5) {
                std::cout << "正在退出..." << std::endl;
        ...
        } else if (currentState_ == "MessageCenter") {
            int order = 0;
            std::string input;
            std::cout << "1. 好友消息" << std::endl;
            std::cout << "2. 群聊消息" << std::endl;
            std::cout << "3. 返回上一界面" << std::endl;
            getline(std::cin, input);
            if (!ReadNum(input, order))
                continue;

            if (order == 1) {
                if (friendlist_.size() == 0) {
                    std::cout << "当前无好友" << std::endl;
                    continue;
                }
                currentState_ = "FriendMessage";
                ClearScreen();
                continue;
            } else if (order == 2) {
        ...
        } else if (currentState_ == "FriendManage") {
            json js = {{"userid", userid_},
                       {"interfaceid", -1},
                       {"interface", "Notice"}};
            waitingback_ = true;
            ...
            int order = 0;
            std::string input;
            std::cout << "1. 好友列表" << std::endl;
            std::cout << "2. 添加好友" << std::endl;
            std::cout << "3. 好友申请" << std::endl;
            std::cout << "4. 查看发送的好友申请" << std::endl;
            ...
            if (order == 1) {
                ...
                for (auto it : friendsendapplylist_) {
                    if (it.second.email_ == email &&
                        it.second.status_ == "Pending") {
                        std::cout << "已发送的好友申请,待处理中" << std::endl;
                        continue;
                    }
                }

                json js = {{"fromid", userid_}, {"targetemail", email}};
                waitingback_ = true;
                SendApply(js);

                waitInPutReady();
            } else if (order == 3) {
                json js = {{"targetid", userid_}};
                waitingback_ = true;
                ListAllApply(js);
                ...
                while (true) {
                    ...
                    getline(std::cin, input);
                    if (!ReadNum(input, fromid) || fromid == 0)
                        break;
                ...
                bool block;
                bool nowblock;
                std::cout << "输入要屏蔽的好友id(退出返回0):";
                getline(std::cin, input);
                if (!ReadNum(input, friendid) || friendid == 0)
                    continue;
                ...
                js = {{"userid", userid_}, {"friendid", friendid}};
                waitingback_ = true;
                DeleteFriend(js);
        ...
        } else if (currentState_ == "GroupManage") {
            json js = {{"userid", userid_},
                       {"interfaceid", -1},
                       {"interface", "Notice"}};
            waitingback_ = true;
            ...
            if (order == 1) {
                ...
                json js = {{"groupname", groupname}, {"userid", userid_}};
                ...
                json js = {{"groupname", groupname}, {"ownerid", userid_}};
        ...
        } else if (currentState_ == "PersonalCenter") {
            json js = {{"userid", userid_},
                       {"interfaceid", -1},
                       {"interface", "Notice"}};
            waitingback_ = true;
            ...
            int order = 0;
            std::string input;
            std::cout << "1. 查看账户信息" << std::endl;
            std::cout << "2. 修改账户密码" << std::endl;
            std::cout << "3. 修改账户名字" << std::endl;
            std::cout << "4. 返回上一界面" << std::endl;
            getline(std::cin, input);
            if (!ReadNum(input, order))
                continue;
            ...
            if (order == 1) {
                ...
                std::cout << "密码: " << password_ << std::endl;
                std::cout << "好友数: " << friendlist_.size() << std::endl;
                ...
                json js = {{"userid", userid_}, {"newpassword", newpassword}};
                waitingback_ = true;
                ChancePassword(js);
                ...
                std::string newname;
                std::cout << "输入新的账户名字(输入0返回): ";
                getline(std::cin, newname);
                if (newname == "0") {
                    ClearScreen();
                    continue;
                }
                json js = {{"userid", userid_}, {"newname", newname}};
                waitingback_ = true;
                ChanceUserName(js);
        ...
        } else if (currentState_ == "FriendMessage") {
            int friendid;
            std::string input;
            ...
            while (true) {
                ...
                if (order == 1) {
                    ...
                    json js = {{"userid", userid_}, {"friendid", friendid}};
                    ...
                    for (int i = 0; i < filelist_.size(); i++) {
                        std::string sendname =
                            friendlist_[filelist_[i].senderid_].name_;
                        std::cout << i + 1 << " " << sendname << ": "
                                  << filelist_[i].filename_
                                  << " 大小: " << filelist_[i].filesize_
                                  << " 时间: " << filelist_[i].timestamp_
                                  << std::endl;
                    }
        ...
        } else if (currentState_ == "GroupMessage") {
            json js = {{"userid", userid_}};
            waitingback_ = true;
            ListGroup(js);
            ...
            for (auto it : grouplist_) {
                std::cout << it.first << " " << it.second.name_ << "      "
                          << it.second.groupuserlist_[userid_].role_;
                if (it.second.newapply_ || it.second.newmessage_)
                    PrintfRed('*');
                std::cout << std::endl;
            }
            ...
            while (true) {
                ...
                std::string userrole;
                if (grouplist_[groupid].groupuserlist_.find(userid_) !=
                    grouplist_[groupid].groupuserlist_.end()) {
                    userrole =
                        grouplist_[groupid].groupuserlist_[userid_].role_;
                } else {
                    currentState_ = "MessageCenter";
                    break;
                }
                ...
                if (order == 1) {
                    ...
                    json js = {{"userid", userid_}, {"groupid", groupid}};
                    ...
                    while (true) {
                        ...
                        bool usermute =
                            grouplist_[groupid].groupuserlist_[userid_].mute_;
                        if (usermute == true) {
                    ...
                    bool usermute =
                        grouplist_[groupid].groupuserlist_[userid_].mute_;
                    if (usermute == true) {
                        std::cout << "你已被禁言,无法发送文件" << std::endl;
                        continue;
                    }
                    ...
                    for (int i = 0; i < filelist_.size(); i++) {
                        std::string sendername =
                            grouplist_[groupid]
                                .groupuserlist_[filelist_[i].senderid_]
                                .username_;
                        std::cout << i + 1 << " " << sendername << ": "
                                  << filelist_[i].filename_
                                  << " 大小: " << filelist_[i].filesize_
                                  << " 时间: " << filelist_[i].timestamp_
                                  << std::endl;
                    ...
                    int friendid;
                    std::cout << "输入要拉进群的好友id(返回输入0): ";
                    getline(std::cin, input);
                    if (!ReadNum(input, friendid))
                        continue;
                    if (friendid == userid_) {
                        std::cout << "你已在群聊中" << std::endl;
                        continue;
                    }
                    if (friendlist_.find(friendid) == friendlist_.end()) {
                        std::cout << "不存在的好友id" << std::endl;
                        continue;
                    }
                    if (grouplist_[groupid].groupuserlist_.find(friendid) !=
                        grouplist_[groupid].groupuserlist_.end()) {
                        std::cout << "好友已在群聊中" << std::endl;
                        continue;
                    }

                    js = {{"friendid", friendid}, {"groupid", groupid}};
                    waitingback_ = true;
                    AddFriendToGroup(js);
                    ...
                    json js = {{"groupid", groupid}, {"userid", userid_}};

                    waitingback_ = true;
                    QuitGroup(js);

                    waitInPutReady();
                    if (grouplist_[groupid].groupuserlist_[userid_].role_ ==
                        "Owner") {
                        waitingback_ = true;
                        DeleteGroup(js);
                    ...
                    int applyid;
                    std::string status;
                    std::cout << "输入要处理的申请用户的id(退出输入0): ";
                    getline(std::cin, input);
                    if (!ReadNum(input, applyid) || applyid == 0)
                        continue;
                    ...
                    js = {{"userid", userid}, {"groupid", groupid}};
                    ...
                    waitInPutReady();
                    if (currentState_ == "MessageCenter")
                        break;
                } else if (order == 12) {
                    currentState_ = "MessageCenter";
                    ClearScreen();
