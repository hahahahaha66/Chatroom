...
const char *getErrnoMsg(int saveErrno) {
    return strerror_r(saveErrno, ThreadInfo::t_errnobuf,
...
Logger::LogLevel initLogLevel() {
    return Logger::INFO; // 默认日志等级, 通过此来修改日志输出
}
...
void Logger::Impl::formatTime() {
    ...
    time_t seconds =
        static_cast<time_t>(now.microSecondsSinceEpoch() /
                            Timestamp::kMicroSecondsPerSecond); // 获取秒数
    int microsecond =
        static_cast<int>(now.microSecondsSinceEpoch() %
                         Timestamp::kMicroSecondsPerSecond); // 获取微秒数
